...
## 📊 ผลสำรวจจริง — วัดครบทุกชิ้นแล้ว (Export Phase 1 เสร็จ 2026-06-23)

Export เต็มเสร็จแล้ว: enrich **373,709 / 373,740** ชิ้น (99.99% · 31 ชิ้นเข้าไม่ได้) + ดึง **transcript ครบ 114/114**

| | จำนวน | **ขนาดรวม (วัดจริง)** |
|---|---:|---:|
| 🎬 **วิดีโอ** | 22,766 | **12,568 GB (~12.6 TB)** ← กิน 84% |
| 🖼️ รูปภาพ | 349,779 | 2,372 GB (~2.37 TB) |
| 🔊 เสียง | 644 | 88 GB |
| 📄 ไฟล์อื่น | 331 | 7.6 GB |
| (timeline/cliplist — Mimir internal) | 189 | — |
| **รวม** | **373,709** | **15,035 GB ≈ 15.0 TB** |

**สิ่งที่ต้องตัดสินใจจากตัวเลขนี้:**
- 🔑 **ต้องการพื้นที่ Google Workspace ~15 TB** (pooled storage) — เช็คกับ Workspace admin ว่าแพ็กเกจ + จำนวน user รองรับไหม (เช่น Business Plus = 5TB/user pooled) **นี่คือ blocker ที่ต้องเคลียร์ก่อน Phase 2**
- วิดีโอคือต้นทุนหลัก (84%) — ถ้าพื้นที่ไม่พอ พิจารณา archive วิดีโอเก่า/บีบอัด proxy แทนต้นฉบับ
- Download URL ต้นฉบับมีครบ **373,260 ชิ้น** → ดึงไฟล์จริงได้เกือบทั้งหมด
- ✅ Transcript มีแค่ **114 ชิ้น** (เฉพาะวิดีโอที่เปิด transcription) — ดึงเก็บครบแล้วใน `out/transcripts/`

> ข้อมูล export อยู่ใน `migration/out/` (gitignore): `items-full.jsonl` (metadata เต็ม 373k), `transcripts/` (114 VTT), `inventory-report.json` · **ควรสำรองไป NAS/bucket เป็น source of truth ก่อน Mimir ปิด**
