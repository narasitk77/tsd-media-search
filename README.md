...
| Vector DB | Qdrant (Docker service) — เก็บใน `data/qdrant/` |
| Embedding | fastembed + `paraphrase-multilingual-MiniLM-L12-v2` (Thai/EN, 384 dims) |
...
| Container | Docker + docker-compose (3 services: app, metadata-tool, qdrant) |
...
ตั้งแต่ v2.4.0 ระบบมี **Semantic Search** ผ่าน Qdrant Vector DB

### สถาปัตยกรรม

```
metadata-tool (Python) ─── embed text ──→ Qdrant (port 6333)
                                           data/qdrant/  ← persistent
```

- **Embedding model**: `paraphrase-multilingual-MiniLM-L12-v2` ผ่าน fastembed (ONNX — ไม่ต้อง PyTorch)
- **รองรับภาษา**: ไทย + อังกฤษ + 50+ ภาษา
- **Vector dimensions**: 384
- **ข้อมูลที่ embed**: ai_title, ai_description, ai_keyword, ai_persons, ai_location, ai_category, ai_subject_tags, ai_event_occasion, exif_photographer

### API Endpoints (ผ่าน `/ai-tool`)

| Method | Path | คำอธิบาย |
|--------|------|----------|
| `GET` | `/api/vector/search?q=คำค้น&limit=20` | ค้นหาเชิงความหมาย |
| `POST` | `/api/vector/index-all` | Index assets ทั้งหมดที่ done |
| `POST` | `/api/vector/index/{item_id}` | Index asset เดี่ยว |
| `GET` | `/api/vector/stats` | สถิติ Vector DB |
| `DELETE` | `/api/vector/{item_id}` | ลบออกจาก index |

### วิธีใช้งานครั้งแรก

```bash
# หลัง batch AI analyze เสร็จแล้ว — สร้าง index ครั้งแรก
curl -X POST http://localhost:8000/api/vector/index-all

# ค้นหาเชิงความหมาย
curl "http://localhost:8000/api/vector/search?q=งานแถลงข่าวทางการเมือง&limit=10"
```

> Asset ที่ผ่าน reanalyze จะถูก index อัตโนมัติทุกครั้ง
...
|---------|--------|--------------|
| 2.4.0 | 21 เม.ย. 69 | Vector Search: Qdrant + Semantic Search รองรับ Thai/EN |
| 2.3.0 | 17 เม.ย. 69 | AI Metadata Tool (Python/FastAPI microservice) |
